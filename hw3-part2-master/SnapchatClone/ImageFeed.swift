        ...
        if snapshot.exists() {
            if let posts = snapshot.value as? [String:AnyObject] {
                user.getReadPostIDs(completion: { (ids) in
                    var readBoolean: Bool = false
                    for (postKey, value) in posts {
                        for item in ids {
                            if item == postKey {
                                readBoolean = true
                            }
                        }
                        postArray.append(Post(id: postKey, username: value["username"] as! String, postImagePath: value["imagePath"] as! String, thread: value["thread"] as! String, dateString: value["date"] as! String, read: readBoolean))
                        readBoolean = false
                    }
